<div>
    <div class="flex items-center justify-between mb-10">
        ...
        <a href="{{route('create')}}" type="button" class="rounded-full bg-pink-600 px-2 py-1 text-xs font-bold text-white shadow hover:bg-pink-500">
            Add Team Mate
        </a>
    </div>
    <div wire:poll>
        ...
        <div wire:key="{{ $member->id }}" class="my-2 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg">
                    {{now()->tz($member->timezone)->format('h:i:s A')}} <span class="text-xs text-gray-500">- {{$member->timezone}}</span>
                </p>
            </div>
